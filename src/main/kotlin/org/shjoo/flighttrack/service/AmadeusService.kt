...
import org.shjoo.flighttrack.dto.*
import org.shjoo.flighttrack.fallback.FallbackData
...
class AmadeusService(
    ...
    private val amadeusAuthService: AmadeusAuthService,
    private val cacheService: CacheService
) {
    private val log = LoggerFactory.getLogger(AmadeusService::class.java)

    // Static IATA → coordinate map (from fallback + common airports)
    ...
    fun findNearestAirport(lat: Double, lng: Double): Pair<String, Pair<Double, Double>>? {
        var best: Pair<String, Pair<Double, Double>>? = null
        var bestDist = Double.MAX_VALUE
        for ((iata, coords) in airportCoords) {
        ...
        // Only return if within ~100km (~1 degree)
        return if (bestDist < 1.0) best else null
    }

    suspend fun searchFlights(from: String, dateFrom: String, dateTo: String): List<FlightResponse> {
        val cacheKey = cacheService.buildKey(from, dateFrom, dateTo)
        cacheService.get(cacheKey)?.let { return it }

        return try {
            val token = amadeusAuthService.getAccessToken()
            val results = searchInspirationFlights(from, dateFrom, token)

            if (results.isNotEmpty()) {
                cacheService.put(cacheKey, results)
                results
            } else {
                log.warn("No Amadeus results for from=$from, using fallback")
                getFallback(from)
            }
        } catch (e: Exception) {
            log.error("Amadeus API call failed: ${e.message}", e)
            getFallback(from)
        }
    }

    private suspend fun searchInspirationFlights(
        from: String,
    ...
    private suspend fun getCoordinates(iata: String, token: String): Pair<Double, Double>? {
        ...
        // Fetch from Amadeus Locations API
        return try {
            val response = amadeusWebClient.get()
                .uri { builder ->
            ...
            if (loc?.geoCode != null) {
                ...
                airportCoords[iata] = coords // cache for next time
                coords
            } else {
                null
            }
    ...
    suspend fun searchAirports(query: String, token: String? = null): List<AmadeusLocationData> {
        return try {
            val t = token ?: amadeusAuthService.getAccessToken()
            val response = amadeusWebClient.get()
                .uri { builder ->
                    builder.path("/v1/reference-data/locations")
                        .queryParam("subType", "AIRPORT")
            ...
            log.error("Airport search failed: ${e.message}")
            emptyList()
        }
    }

    private fun getFallback(from: String): List<FlightResponse> {
        log.info("Using fallback data for from=$from")
        return if (from.equals("ICN", ignoreCase = true)) {
            FallbackData.getSeoulFallback()
        } else {
            emptyList()
        }
    }
